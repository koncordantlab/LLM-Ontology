...
def raw_relations_to_graph(file: str, g : Graph) -> Graph:
    """
...
def create_graph_and_write(INPUT_FILE, OUTPUT_FILE):
    print(INPUT_FILE)
    print(OUTPUT_FILE)
    graph = raw_relations_to_graph(INPUT_FILE, Graph())
    print(graph)
    serialize(graph, OUTPUT_FILE)
    print(graph)

def serialize(graph, OUTPUT_FILE):
    graph.serialize(OUTPUT_FILE, format='pretty-xml')

def extend_graph_and_write(INPUT_FILE, OUTPUT_FILE):
    g = Graph()
    g.parse("outfiles/preExtension.xml")
    graph = raw_relations_to_graph(INPUT_FILE, g)
    serialize(graph, OUTPUT_FILE)

def merge_graphs_write(input_file_1, input_file_2, output_file):
    with open(input_file_1, 'r') as f:
        relations = f.read()

    with open(input_file_2, 'a') as f:
        f.write(relations)
    g = raw_relations_to_graph(input_file_2, Graph())
    serialize(g, output_file)
